...
def refuse_when_missing(identifier, name):
    ...
    raise FileNotFoundError(
        "%s has no asset named %s; it holds %s"
...
def read_text(identifier, name):
    return as_dos_text(
        read_bytes(identifier, name).decode(ENCODING, UNSUPPORTED_CHARACTERS)
    )
...
def refuse_unfilled(text, values):
    ...
    if missing:
        raise KeyError("nothing was given for %s" % ", ".join(missing))
...
def prepared(identifier, name, values):
    return filled(read_text(identifier, name), values).encode(
        ENCODING, UNSUPPORTED_CHARACTERS
    )
